...
dataset_dict = {
    ...
    "ARCHS4_50000_input": "../data/processed/ARCHS4/human_matrix_v9_filtered_n50000x962_step2_input.f",
    "ARCHS4_50000_output": "../data/processed/ARCHS4/human_matrix_v9_filtered_n50000x25312_step2_output.f",
    ...
    "step1": "../output/{}/prediction/y_pred.txt"
    ...
    if opt.ispredicting == False:
        ...
        for epoch in range(opt.epoch_resume, opt.n_epochs):
            ...
            if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
                ...
                torch.save(E.state_dict(), model_folder+"E_%d.pth" %
                        (epoch))
                save(val_pred, folder=prediction_folder, filename="y_pred.txt", shuffle=opt.shuffle)
                save(val_labels, folder=prediction_folder, filename="y_true.txt", shuffle=opt.shuffle)
        ...
        if opt.eval_input_dataset_name == "step1":
            data_fileA = dataset_dict[opt.eval_input_dataset_name].format(opt.exp_index)
            data_fileB = dataset_dict[opt.eval_output_dataset_name]
        ...
        # load model
        E.load_state_dict(torch.load(model_folder+"E_%d.pth" %
        ...
        save(y_true, folder=prediction_folder, filename="y_true.txt", shuffle=opt.shuffle)
        save(predictions, folder=prediction_folder, filename="y_pred.txt", shuffle=opt.shuffle)
...
def train_step(model, criterion, optimizer, input_tensor_train, output_tensor_train):
    ...
    for i, (batchA, batchB) in enumerate(zip(dataloaderA, dataloaderB)):
        ...
        # Print log
        print("Process Training Batch: [{}/{}]".format(i, len(dataloaderA)), end='\r')  
        # print("print?")
        tr_loss += loss_E.item()
